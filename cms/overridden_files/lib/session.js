...
function signinWithUser(user, req, res, onSuccess) {
	if (arguments.length < 4) {
		throw new Error(
			"keystone.session.signinWithUser requires user, req and res objects, and an onSuccess callback."
		);
	}
	if (typeof user !== "object") {
		throw new Error(
			"keystone.session.signinWithUser requires user to be an object."
		);
	}
	if (typeof req !== "object") {
		throw new Error(
			"keystone.session.signinWithUser requires req to be an object."
		);
	}
	if (typeof res !== "object") {
		throw new Error(
			"keystone.session.signinWithUser requires res to be an object."
		);
	}
	if (typeof onSuccess !== "function") {
		throw new Error(
			"keystone.session.signinWithUser requires onSuccess to be a function."
		);
	}
...
var doSignin = function(lookup, req, res, onSuccess, onFail) {
	if (!lookup) {
		return onFail(
			new Error(
				"session.signin requires a User ID or Object as the first argument"
			)
		);
	}
	...
	if (typeof lookup.email === "string" && typeof lookup.password === "string") {
		...
		var emailRegExp = new RegExp(
			"^" + utils.escapeRegExp(lookup.email) + "$",
			"i"
		);
		...
		var userId =
			lookup.indexOf(":") > 0 ? lookup.substr(0, lookup.indexOf(":")) : lookup;
		var passwordCheck =
			lookup.indexOf(":") > 0 ? lookup.substr(lookup.indexOf(":") + 1) : false;
		User.model.findById(userId).exec(function(err, user) {
			if (
				user &&
				(!passwordCheck || scmp(passwordCheck, hash(user.password)))
			) {
				postHookedSigninWithUser(user, req, res, onSuccess, onFail);
			} else {
...
exports.persist = function(req, res, next) {
	...
	if (
		keystone.get("cookie signin") &&
		!req.session.userId &&
		req.signedCookies["keystone.uid"] &&
		req.signedCookies["keystone.uid"].indexOf(":") > 0
	) {
		exports.signin(
			req.signedCookies["keystone.uid"],
			req,
