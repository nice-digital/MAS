...
Item.schema.post("save", async function(doc, next) {
	...
	try {
		...
		logger.error(`An error occurred finding item: ${err.message}. Looking for item ${doc._id}`);
		return next(new Error(`An error occurred finding item: ${err.message}. Looking for item ${doc._id}`));
	}
